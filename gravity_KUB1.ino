// --------------------- НАСТРОЙКИ ----------------------
//https://github.com/AlexGyver/GravityPixels



#define PIXEL_AMOUNT 50      // число "живых" пикселей 50
#define G_CONST 9.81         // ускорение свободного падения

#define BRIGHTNESS 100        // яркость (0 - 255)150

#define MATR_X 8            // число светодиодов по х 16
#define MATR_Y 8            // число светодиодов по у 16
#define MATR_X_M 80         // размер матрицы в миллиметрах х 160
#define MATR_Y_M 80         // размер матрицы в миллиметрах у 160

#define PIXELZISE 10        // размер пикселя мм 10
...
int offsets[6] = { -1475, -2248, 5638, 62, -21, 10}; //требует калибровку из скетча 
//int offsets[6] = {-1542, -2365, 5654, 61, -20, 17 };
  ...
  if (millis() - loopTimer > MIN_STEP) {
    ...
    for (byte i = 0; i < PIXEL_AMOUNT; i++) {
      ...
      if (GLOW) {
        ...
        glowDraw(nowDistX + 1, nowDistY, GLOW_FADE1);        // нарисовать точку //+1
...
void pixelDraw(byte x, byte y, byte colorNum) {
  if (y % 2 == 0)                                                         // если чётная строка
    ...
    strip.setPixelColor(y * MATR_X + MATR_X - x -1, colors[colorNum]);   // заливаем в обратном порядке //-1
}
